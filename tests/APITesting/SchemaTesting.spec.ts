...
//1) 
test('Validate JSON response with schema1', {tag: ['@api']}, async ({ request }) => {
...
test('Validate JSON response with schema2', {tag: ['@api']},async ({ request }) => {
    const response = await request.get('https://jsonplaceholder.typicode.com/posts/1');
    const responsebody = await response.json();

    // Define the JSON schema use json shema converter to get below format if not available in document
    const schema = {
        type: 'object',
        properties: {
            userId: { type: 'integer' },
