...
test('Public API - No Auth', {tag: ['@api']}, async ({ request }) => {
...
// 2. Basic Auth
test('Basic Auth - HTTPBin', {tag: ['@api']}, async ({ request }) => {
...
// 3. Bearer Token Auth (Get github repositories created by user)

test('Verify Bearer Token Authentication', {tag: [ '@api']}, async ({ request }) => {
  test.fail();
  const bearerToken = "abcd"; //Enter correct data and remove above line to verify the exact scenario
...
test('Bearer Token Auth', {tag: ['@api']}, async ({ request }) => {
  test.fail();
  const token = 'abcd2'; // Replace with a real token and remove above line
...
test('Verify API Key Authentication', {tag: [ '@api']}, async ({ request }) => {
...
test('API Key Auth - Used in Header', {tag: ['@api']}, async ({ request }) => {
