...
test('Single file upload', async ({ page }) => {
  ...
  await page.locator('#singleFileInput').setInputFiles('uploads/Test1.txt'); //for macc use \
...
test('Multiple file upload', async ({ page }) => {
  ...
  const msg : string | null= await page.locator('#multipleFilesStatus').textContent();
  expect(msg).toContain('Test1.txt');
