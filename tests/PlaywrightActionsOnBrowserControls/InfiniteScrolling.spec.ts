...
test('Infinite scroll on booksbykilo.in', async ({ page }) => {
  ...
  while (true) { //since the end of page is not known use true
    // Scroll to the bottom
    /*await page.evaluate(() => {
      window.scrollTo(0, document.body.scrollHeight); // 0 start of page and scrollheight end of visible page
    });
    */   //OR
...
test('Find A Book in the page', async ({ page }) => {
  ...
  while (true) {
    ...
    const titles : string[] = await page.locator('#productsDiv h3').allTextContents();
...
test('Count Total Books in the page', async ({ page }) => {
  ...
  while (true) {
    ...
    // Get all book titles currently loaded on the page
    const books : Locator[] = await page.locator('#productsDiv h3').all();
