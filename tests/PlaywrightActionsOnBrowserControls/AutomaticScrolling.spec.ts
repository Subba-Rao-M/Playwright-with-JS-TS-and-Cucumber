...
test('Scrolling inside dropdrown', async ({ page }) => {
  ...
  const option: Locator = page.locator('#dropdown div:nth-child(100)');
...
test('Scrolling inside the table', async ({ page }) => {
  ...
  const name: string = await page.locator('tbody tr:nth-child(10) td:nth-child(2)').innerText(); //Automatic scrolling - vertical
  console.log("Last Name from 10th Row & 2nd Column :", name); //Kelly

  const email: string = await page.locator('tbody tr:nth-child(10) td:nth-child(9)').innerText(); //Automatic scrolling - Horizantal
