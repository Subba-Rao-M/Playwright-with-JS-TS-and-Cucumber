...
while(hasmorepages)
{
    ...
    const isDisabled : string | null=await nextButton.getAttribute('class'); // dt-paging-button disabled next

    if(isDisabled?.includes('disabled')) // ? to check if its string or null
    {
        hasmorepages=false;
    }
    else{
