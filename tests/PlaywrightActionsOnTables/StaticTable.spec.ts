...
test("static web table", async ({ page }) => {
    ...
    //Chaining of locator using already captured locator
    ...
    {
        const cols: string[] = await row.locator('td').allInnerTexts(); //array format
        console.log(cols.join('\t')); // convert to simple format like single line
    }
    ...
    for (let row of allRowData.slice(1))   // slice(1) --> skip header row 
    {
        ...
        const authorName : string = await row.locator('td').nth(1).innerText(); // Capture Author name 
        const bookName : string = await row.locator('td').nth(0).innerText(); // Capture Book name 
    ...
    for (let row of allRowData.slice(1))   // slice(1) --> skip header row 
    {
        ...
        const price : string = await row.locator('td').nth(3).innerText(); // You can use single statement instead of using above 2 statements to get price
