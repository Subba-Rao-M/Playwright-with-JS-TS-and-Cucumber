...
/**
 * Relationship between elements using locator
 * allows navigation between elements using nodes using relationship
 * navigation between nodes possible in both directions
 * Use * to get all values for axes methods :: *
 */
...
const germanyCell: Locator=page.locator("//td[text()='Germany']/self::td"); // Points to same field where value is Germany for demo purpose
...
console.log(await parentRow.textContent());
 ...
 await expect(secondRowCells).toHaveCount(3);
  ...
  const table:Locator=page.locator("//td[text()='Germany']/ancestor::table"); // to get all ancestor use * //td[text()='Germany']/ancestor::*
...
// 5. descendant axis - Get all <td> elements under the table
  ...
  //following will have both following sibiling and siblings childs
  // following sibiling will have only siblings and no further child elements from sibilings
  //Same w.r.t preceding and preceding-sibling 
