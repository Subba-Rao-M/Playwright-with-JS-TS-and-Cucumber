...
async function selectDateOfBirth(page: Page, birthYear: string, birthMonth: string, birthDate: string) {
    ...
    // Select Date
    const dateCells : Locator[] = await page.locator('table.ui-datepicker-calendar td a').all();
...
test('Dummy Ticket Booking', async ({ page }) => {
    ...
    await page.locator('[name="travname"]').fill('Subba');
    await page.locator('#travlastname').fill('Rao');

    // Select Date Of Birth
    const birthYear = "1986";
    const birthMonth = "Nov";
    const birthDate = "29";
    await page.locator('#dob').click();
    await selectDateOfBirth(page, birthYear, birthMonth, birthDate);

    // Verify date was selected
    const dobValue : string = await page.locator('#dob').inputValue();
    ...
    const requiredYear : string = "2026";
    const requiredMonth : string= "Jan";
    const requiredDate : string= "25";
    ...
    const departValue : string = await page.locator('#departon').inputValue();
    ...
    const apptYear : string = "2026";
    const apptMonth : string= "Jan";
    const apptDate : string= "28";
    ...
    // Billing Details //
    await page.locator('#billname').fill('Subba Rao');
    ...
    await expect(page.locator('#billname')).toHaveValue('Subba Rao');
    ...
    const productName : Locator = page.locator('.product-details');
    await expect(productName).toHaveText("Dummy ticket for Visa Application");

    const productPrice : Locator = page.locator('.shop_table.woocommerce-checkout-review-order-table tfoot tr:nth-child(2) td');
