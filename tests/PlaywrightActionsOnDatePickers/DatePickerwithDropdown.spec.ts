...
async function selectDate(page: Page, targetYear: string, targetMonth: string, targetDate: string) {
  // Select the year
  const yearDropdown: Locator = page.locator('select.ui-datepicker-year');
  await yearDropdown.selectOption({ label: targetYear });

  // Select the month
  const monthDropdown: Locator = page.locator('select.ui-datepicker-month');
  await monthDropdown.selectOption({ label: targetMonth });

  // Click on the desired date
  const allDates: Locator[] = await page.locator('table.ui-datepicker-calendar a').all();
