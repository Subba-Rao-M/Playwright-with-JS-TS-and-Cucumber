...
test("Single Select Drop down", async ({ page }) => {
    ...
    //await page.locator('#country').selectOption({index:3});   // by using index and index starts from 0


    //2) check number of options in the dropdown(count)

    const dropdownOptions: Locator = page.locator('#country>option'); // or > can be replaced with space #country option
